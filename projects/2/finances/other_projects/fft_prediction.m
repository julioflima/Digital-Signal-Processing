...
clc
    ...
    ordinate = data.price(1:end-daysInterpol);
    abscissa1 = (1:(samp))';
    abscissa = (1:(samp-daysInterpol))';
    samp1 = samp;
    samp = samp - daysInterpol;
    ...
    fourierInverse=ifft(fourierDiff);
    figure
    plot(fourierInverse-diff)
    title('IFFT of Cleaned Difference and Original Difference') 
    
    
    % Interpolation
    for n=samp:(samp+daysInterpol)
    YY(n)=0;
     for k=1:samp
    ...
    figure
    plot(1:(samp+daysInterpol), data.price);
    title('Stock Prive vs. Theoretical Price')
    old = [regCurve'  zeros(daysInterpol,1)']';
    %Generate the Regression Curve.
        switch order ~=0
        case order == 2
    ...
    hold on;
    plot((1:(samp+daysInterpol)), total);
    hold off;
